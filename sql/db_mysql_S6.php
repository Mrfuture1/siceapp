...
function connect($Sid = 'buptsice1', $User = 'root', $Password='52hesha520?', $Permanent = FALSE, $Ip = '127.0.0.1', $port = '3306')
 {
 if ($this->_connection != null)
  {
 ...
 {  
  ...
  else
  {
      ...
      //构造获取该页面数据的SQL语句
      $newsql = strstr(strtolower($Sql), ' order by ', true); 

      $SqlPage =$newsql . " and $tablename.$idkey in( $strid ) " . $orderby;
      //echo $SqlPage."<br>";
 ...
 function show_LinkPage()
 {
   ...
   // if( $this->_AllPage <=1 )
 	...
 	//	$linkPage .= "<table width=80% border=0 align=center ><tr><td>共有记录 $this->_AllNumber 行</td></tr></table>";
  	//	return $linkPage;
  //	}
  	//取当前的uri:
//?  	$uri= $_SERVER["REQUEST_URI"]; //形如 "/index.php?p=222&q=biuuu"
  	//检查有无？号
    $uri = "./subPage.php?id=".$_REQUEST[id]."&"."StartPage=".$_REQUEST[StartPage];
  	$pos = strpos ($uri,"?");
  	$CurrentParam = null;
  	if ($pos>0) //若有？号
  	{
  	...
  	$FontSize = "0.8";
  	$linkPage.= "<table width=63% border=0 align=center ><tr><td  valign = \"middle\"   style=\"font-size:".$FontSize."em\" align=left>";
  	$linkPage.= "共 $this->_AllNumber 行(分 $this->_AllPage 页)&nbsp&nbsp当前第$this->_StartPage 页 ";

  	$linkPage.=  "</td><td  align=right style=\"font-size:".$FontSize."em\" ><table border=0 cellspacing=1 width=100%><tr><td valign = \"middle\" align=center  style=\"font-size:".$FontSize."em\">";
  	if ($this->_StartPage!=1)
  	{
  		$linkPage.=  "<a href=".$CurrentParam."StartPage=1>首页</a> ";
  		$Prepage = $this->_StartPage-1;
  		$linkPage.=  "<a href=".$CurrentParam."StartPage=$Prepage>上一页&nbsp&nbsp</a> ";
  	}
  	else
  	{
  		$linkPage.=  "<font style=\"color:#c0c0c0\">首页&nbsp&nbsp上一页&nbsp&nbsp</font>";
  	}
  	if ($this->_StartPage!=$this->_AllPage)
  	{
  		$Nextpage = $this->_StartPage+1;
  		$linkPage.=  "<a href=".$CurrentParam."StartPage=$Nextpage>下一页&nbsp&nbsp</a> ";
  		$linkPage.=  "<a href=".$CurrentParam."StartPage=$this->_AllPage>末页</a> ";
  	}
  	else
  	{
  		$linkPage.=  "<font style=\"color:#c0c0c0\">下一页&nbsp&nbsp末页</font>";
  	}

  	//显示go按钮
  	$linkPage.=  "</td> <td  valign = \"middle\" align=center style=\"font-size:".$FontSize."em\"><br>";
  	$linkPage.=  "<form action='".$CurrentParam1."' method=post> <select name=StartPage>";
    if ($this->_AllPage<100){
      	for ($i=1;$i<=$this->_AllPage;$i++)
      	{
      		if ($i==$this->_StartPage) $linkPage.=  "<option value=$i selected>$i</option>";
        		else $linkPage.=  "<option value=$i>$i</option>";
       	}
    }
    else
    {
        ...
        while ($np>100){ $np = (int)($np / 10) ; $k *= 10;}
      	...
      	{
      	    $ip = $i * $k;
      		if ($ip<=$this->_StartPage && $ip + $k > $this->_StartPage ) $linkPage.= "<option value=$ip selected>$ip</option>";
        		else $linkPage.=  "<option value=$ip>$ip</option>";
       	}
    }
  //	$linkPage.=  "</select> <input type=hidden value=$this->_PageSize name=PageSize> <input type=submit value='go'> </form>";
  	$linkPage.=  "</select><input type=submit value='go'> </form>";
    $linkPage.=  "</td></tr></table>";
  	$linkPage.= "</td></tr></table>";
